...
const translations = {
  ...
  'mcp.quick_import': { zh: '快速导入', en: 'Quick Import' },
  'mcp.import_desc': { zh: '将外部 MCP 配置快速导入到 {target}', en: 'Quickly import external MCP config into {target}' },
  'mcp.import_input': { zh: '粘贴配置、URL、命令或 CLI 片段', en: 'Paste config, URL, command, or CLI snippet' },
  'mcp.import_placeholder': {
    zh: '支持 URL、命令、mcpServers JSON、VS Code servers JSON、OpenCode mcp JSON、Codex TOML、Claude CLI add/add-json',
    en: 'Supports URL, command, mcpServers JSON, VS Code servers JSON, OpenCode mcp JSON, Codex TOML, and Claude CLI add/add-json',
  },
  'mcp.detected_format': { zh: '识别格式', en: 'Detected format' },
  'mcp.import_unrecognized': { zh: '未识别', en: 'Unrecognized' },
  'mcp.import_waiting': { zh: '等待输入', en: 'Waiting for input' },
  'mcp.import_parse_failed': { zh: '未识别出可导入的 MCP 服务', en: 'No importable MCP servers detected' },
  'mcp.import_preview_empty': { zh: '粘贴内容后将在此显示导入预览', en: 'Import preview will appear here after you paste content' },
  'mcp.import_notes': { zh: '导入提示', en: 'Import notes' },
  'mcp.import_no_warnings': { zh: '没有兼容性警告', en: 'No compatibility warnings' },
  'mcp.import_footer': { zh: '导入后会追加到当前分组；重名项会自动重命名。', en: 'Imported servers are appended to the current target; duplicate names are renamed automatically.' },
  'mcp.import_action': { zh: '导入到当前分组', en: 'Import into current target' },
  'mcp.copy': { zh: '复制到…', en: 'Copy to…' },
  'mcp.copy_target.codex': { zh: 'Codex (config.toml)', en: 'Codex (config.toml)' },
  'mcp.copy_target.cursor': { zh: 'Cursor (mcp.json)', en: 'Cursor (mcp.json)' },
  'mcp.copy_target.gemini': { zh: 'Gemini CLI', en: 'Gemini CLI' },
  'mcp.copy_target.claude_settings': { zh: 'Claude Code settings.json', en: 'Claude Code settings.json' },
  'mcp.copy_target.claude_global': { zh: 'Claude Code 全局 (~/.claude.json)', en: 'Claude Code global (~/.claude.json)' },
  'mcp.copy_done': { zh: '已复制「{name}」到 {target}', en: 'Copied "{name}" to {target}' },
  'mcp.copy_failed': { zh: '复制失败: {error}', en: 'Copy failed: {error}' },
  'mcp.transport': { zh: '传输方式', en: 'Transport' },
  'mcp.headers': { zh: '请求头（每行 Header: Value）', en: 'Headers (Header: Value per line)' },
